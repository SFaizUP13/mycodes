...
RoutingHelper::RoutingHelper ()
  : m_TotalSimTime (100.01),
    m_protocol (0),                  //I have changed from 0 to 1
    m_port (9),
    m_nSinks (0),
    m_log (0)
{
...
{
 // Ipv4ListRoutingHelper list;                                                 <<<<<<<<<<<<<<<<<<<  18/06    <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
RoutingHelper::ReceiveRoutingPacket (Ptr<Socket> socket)
{
  ...
  while ((packet = socket->RecvFrom (srcAddress)))
    {
      // application data, for goodput                                        <<<<<<<<<<<<<<<<<  No need   18/06  <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
 ...
 */
class ConfigStoreHelper                                                                 // <<<<<<<<<<<<<<<<<<<<<<<<<<  No need, its just configuration store <<<<<<<<<<<<<<<
{
public:
  /**
   * \brief Constructor
...
private:
  ...
  /**
   * \brief Set up generation of IEEE 1609 WAVE messages,                                      <<<-----------------<<< BSM setup is here <<<<<<<<--------------
  ...
  double m_txSafetyRange2; ///< range 10
  double m_txSafetyRange3; ///< range
  std::vector <double> m_txSafetyRanges; ///< list of ranges
  std::string m_exp; ///< exp
  int m_cumulativeBsmCaptureStart; ///< capture start
};

VanetRoutingExperiment::VanetRoutingExperiment ()                                    // <<<<<<<<<<<<<--------<<<<<<<------  Input Parameters -- ---<<<<<<<<<<<<<<<<<<
  : m_port (999),
    ...
    // AODV
    m_protocol (0),                     //I have changed it from 1 to 0
    // Two-Ray ground
    m_lossModel (3),
    m_fading (0),
    m_lossModelName (""),
    ...
    m_gpsAccuracyNs (40),
    m_txMaxDelayMs (10),
    m_routingTables (1),                    // I have changed it from 0 to 1
    m_asciiTrace (),
    m_pcap (0),
    ...
    m_txSafetyRange2 (500.0),
	m_txSafetyRange3 (1000.0),
    m_txSafetyRanges (),
    m_exp (""),
...
static ns3::GlobalValue g_nNodes ("VRCnNodes",
                                  ...
                                  ns3::UintegerValue (156),                                               //<<<<<<<<<<<< I have Changed it from 156 to 10
                                  ns3::MakeUintegerChecker<uint32_t> ());
...
static ns3::GlobalValue g_routingTables ("VRCroutingTables",
                                         ...
                                         ns3::UintegerValue (0),                                           // <<<<<<<<<<< I have Changed it from 0 to 1
                                         ns3::MakeUintegerChecker<uint32_t> ());
...
static ns3::GlobalValue g_traceFile ("VRCtraceFile",
                                     ...
                                     ns3::StringValue ("parisMobilityFile2.xml"),//"./src/wave/examples/low99-ct-unterstrass-1day.filt.7.adj.mob"),
                                     ns3::MakeStringChecker ());
...
void
VanetRoutingExperiment::ProcessOutputs ()                                                  //<<<<<<<<<<<<<<<<<<<<-------  OUTPUT Config  ----<<<<<<<<<<<<
{
  // calculate and output final results
  double bsm_Cpdr1 = m_waveBsmHelper.GetWaveBsmStats ()->GetCumulativeBsmPdr (1);
  double bsm_Cpdr2 = m_waveBsmHelper.GetWaveBsmStats ()->GetCumulativeBsmPdr (2);
  double bsm_Cpdr3 = m_waveBsmHelper.GetWaveBsmStats ()->GetCumulativeBsmPdr (3);
...
VanetRoutingExperiment::Run ()
{
  ...
  CheckThroughput ();
  AnimationInterface anim ("BSM_Anim2.xml");                                                        //<<<<<<<<<<< ----  I have added NetAnim
...
VanetRoutingExperiment::CheckThroughput ()
{
  ...
  uint32_t packetsReceived = m_routingHelper->GetRoutingStats ().GetRxPkts ();                   // <<<<<<<<<<<<<<<<<<<< ----   kbps calculation   ---- <<<<<<<<<<<<
  double kbps = (bytesTotal * 8.0) / 1000;
  double wavePDR = 0.0;                                                                           // <<<<<<<<<<<<<<<<<<<< ----   wavePDR calculation   ---- <<<<<<<<<<<<
  ...
  // calculate MAC/PHY overhead (mac-phy-oh)                                                      //<<<<<<<<<<<<<<<<<<< ------  MAC/PHY Overhead calculation <<<<<<<<
  // total WAVE BSM bytes sent
  uint32_t cumulativeWaveBsmBytes = m_waveBsmHelper.GetWaveBsmStats ()->GetTxByteCount ();
  uint32_t cumulativeRoutingBytes = m_routingHelper->GetRoutingStats ().GetCumulativeTxBytes ();
  uint32_t totalAppBytes = cumulativeWaveBsmBytes + cumulativeRoutingBytes;
  ...
  if (m_log != 0 )
    {
      NS_LOG_UNCOND ("At t=" << (Simulator::Now ()).GetSeconds () << "s BSM_PDR1=" << wavePDR1_2 << " BSM_PDR2=" << wavePDR2_2 << " BSM_PDR3=" << wavePDR3_2 << " Goodput=" << kbps << "Kbps" /*<< " MAC/PHY-OH=" << mac_phy_oh*/);
    }
  ...
  for (int index = 1; index <= 10; index++)
    {
      m_waveBsmHelper.GetWaveBsmStats ()->SetExpectedRxPktCount (index, 0);              //<<<<<<<<<<<<<<<<<< ------  did not understand the purpose of expectedRx/TxPkt <<<<<<
      m_waveBsmHelper.GetWaveBsmStats ()->SetRxPktInRangeCount (index, 0);
    }

  double currentTime = (Simulator::Now ()).GetSeconds ();
  if (currentTime <= (double) m_cumulativeBsmCaptureStart)                             //<<<<<<<<<<<<<<<<<< ------  did not understand this  <<<<<<<<<<<< <<<<<
    {
      for (int index = 1; index <= 10; index++)
        {
...
VanetRoutingExperiment::SetConfigFromGlobals ()
{
  ...
  // This may not be the best way to manage program configuration                                    <<<<<<<<<< ------ Configuration Global values setting ----  <<<<<<<
...
void                                                                                    //  <<<<<<<<<<<<<<< this is used just in case if we want to get the stored configurations
...
void
VanetRoutingExperiment::CommandSetup (int argc, char **argv)                                       //<<<<<<<<<<<<<<< --------  COMMAND LINE Setup---- <<<<<<<<<<<
{
  CommandLine cmd;
  double txDist1 = 50.0;
  double txDist2 = 500.0;
  double txDist3 = 1000.0;
...
void
VanetRoutingExperiment::SetupAdhocMobilityNodes ()                                                 // <<<<<<<<<<<<<<<<<<<<<< ------------ Setting Mobility --- <<<<<<<<<
{
  if (m_mobility == 1)
    {
...
void
VanetRoutingExperiment::SetupScenario ()                                                // <<<<<<<<<<<  -------- Setting Scenarios -------<<<<<<<<<<<<<<<
{
