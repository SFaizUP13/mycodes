                  ...
                  if (receiverMoving == 1)
                    {
                      ...
                      if (distSq > 0.0)
                        {
                          // dest node within range?                                     <<<<<<<<<<<<<<<<<<< I did not understand this clearly  <<<<<<<<<<<<<<
...
NS_OBJECT_ENSURE_REGISTERED (BsmApplication);                           // <<<<<<<-----   Creating the BsmApplication <<<<<<
...
BsmApplication::BsmApplication ()                                    // <<<<<<< ------ Declaring variables/parameters <<<<<<<<<
  : m_waveBsmStats (0),
    m_txSafetyRangesSq (),
    m_TotalSimTime (Seconds (10)),
    m_wavePacketSize (200),
...
// Application Methods                                                                  <<<<<<<<<<<<<  defining application method <<<<<<<<<<
void BsmApplication::StartApplication () // Called at time specified by Start
{
  NS_LOG_FUNCTION (this);
  ...
  // every node broadcasts WAVE BSM to potentially all other nodes                              <<<<<<<<<<<<  Bsm broadcast   <<<<<<<<<<<<<<<
  Ptr<Socket> recvSink = Socket::CreateSocket (GetNode (m_nodeId), tid);
  recvSink->SetRecvCallback (MakeCallback (&BsmApplication::ReceiveWavePacket, this));
  InetSocketAddress local = InetSocketAddress (Ipv4Address::GetAny (), wavePort);
  recvSink->BindToNetDevice (GetNetDevice (m_nodeId));
                       ...
                       Time txMaxDelay)
{
  ...
  m_waveInterval = waveInterval;
  m_gpsAccuracyNs = gpsAccuracyNs;
  int size = rangesSq.size ();                                                // <<<<<<<<<<<<<<<<< ------- Ranges Square  <<<<<<<<<<<<
  m_waveBsmStats = waveBsmStats;
  m_nodesMoving = nodesMoving;
  m_chAccessMode = chAccessMode;
  m_txSafetyRangesSq.clear ();
  ...
  if (pktCount > 0)
    {
      ...
      Ptr<MobilityModel> txPosition = txNode->GetObject<MobilityModel> ();
      NS_ASSERT (txPosition != 0);
                                                              // <<<<<<<<<<<<<<< ----This section Coding added by Dr. Nadjib  <<<<<<<<<<
      int senderMoving = m_nodesMoving->at (txNodeId);
      if (senderMoving != 0)
        {
          //
          std::ostringstream msg;
          msg << "nodes[" << txNodeId << "]:";      //  <<<<<<<<<<<<<<<<<<<<<<     -----------  The moving nodes  <<<<<<<<<<<
          // STEP 1: find the number of nodes that can be detected by the embaded sensors (radar/lidar/camera/...)
          int nRxNodes = m_adhocTxInterfaces->GetN ();
          for (int i = 0; i < nRxNodes; i++)                   // <<<<<<<<<  ----  Check the nodes within the range  <<<<<<<<<<<<<
            {
              Ptr<Node> rxNode = GetNode (i);
              int rxNodeId = rxNode->GetId ();
              if (rxNodeId != txNodeId)                        // <<<<<<<<  ---- check if its not the transmitter itself  <<<<<<<<
                {
                  double distSq = MobilityHelper::GetDistanceSquaredBetween (txNode, rxNode);   // <<<<<<<<<<  ----- Calculate the distance b/w TxNode and RxNode  <<<<<<<<<<
               //   std::cout << "TX:"<< txNode->GetId () << "RX:" << rxNode->GetId () << "==>" << distSq << "\n";    <<<<<<<<<<  --- prints the distance b/w Tx and Rx <<<<<
          ...
          //socket->Send (packet);                                                                  <<<<<<<<<<<<<<<<<<<<<< ---------------- To add the payload --- <<<<<<<
                                                                             // <<<<<<<<<<<<<<<<   ---- Upto here section <<<<<<<<<<<<<<<<<<
          socket->Send (Create<Packet> (pktSize));
          // count it
          m_waveBsmStats->IncTxPktCount ();
